...
# какое поле-документ под какую задачу — сначала по ИМЕНИ ФАЙЛА (сотрудники путают поля!),
# потом по ярлыку поля Bitrix. Ключи — подстроки в нижнем регистре.
DOC_PURPOSE = {
    "avr":       ("авр", "накладн", "выполнен", "кс-2", "кс-3", "кс2", "кс3", "акт вып"),  # выполнено
    "article":   ("тех требован", "тех. требован", "техтребован", "тех-требован", "тз"),   # вид работ → статья
    "contract":  ("договор", "контракт", "подряд"),                                        # условия
    "invoice":   ("счёт", "счет", "инвойс", "invoice"),                                    # сумма/№ счёта
}
# порядок проверки при классификации по имени: сперва специфичные (АВР/договор), потом общие
_PURPOSE_ORDER = ("avr", "contract", "article", "invoice")


def classify_doc(label, name):
    """Что это за документ: приоритет ИМЕНИ ФАЙЛА над ярлыком поля (сотрудники кладут не в то поле).
    Возвращает 'avr'/'contract'/'article'/'invoice' или None."""
    nm = (name or "").lower()
    for purp in _PURPOSE_ORDER:                      # 1) по имени файла
        if any(k in nm for k in DOC_PURPOSE[purp]):
            return purp
    lb = (label or "").lower()
    for purp in _PURPOSE_ORDER:                      # 2) по ярлыку поля
        if any(k in lb for k in DOC_PURPOSE[purp]):
            return purp
    return None
...
def field_by_purpose(item, purpose):
    ...
    Классификация по имени файла + ярлыку поля (устойчиво к ошибкам сотрудников в раскладке)."""
    labels = field_labels()
    paths = []
    for field, url, name in file_fields(item):
        if classify_doc(labels.get(field, ""), name) == purpose:
            try:
...
NAKOPITEL_JSON_SCHEMA = {
    ...
    "properties": {
        ...
        "barter": {"type": "boolean"}, "barter_sum": _N, "vypolneno_sum": _N,
        "ochered": _S, "object": _S, "account": _S, "bin": _S, "article": _S,
        "doc_kinds": _S, "notes": _S,
    },
    "required": ["contract_no", "contract_date", "total", "currency", "nds", "avans_pct",
                 "avans_sum", "retention_pct", "retention_sum", "barter", "barter_sum",
                 "vypolneno_sum", "ochered", "object", "account", "bin", "article",
                 "doc_kinds", "notes"],
    "additionalProperties": False,
}
NAKOPITEL_SCHEMA = NAKOPITEL_JSON_SCHEMA   # совместимость со старым импортом
NAKOPITEL_INSTRUCTION = (
    "Ты финансовый контролёр стройхолдинга (Казахстан). Перед тобой ОДИН ИЛИ НЕСКОЛЬКО документов по одной "
    "заявке на оплату: это может быть договор подряда/поставки, счёт, тех.требование, АВР/КС-2/КС-3 (акт "
    "выполненных работ), или ВСЁ ЭТО В ОДНОМ комбинированном файле. НЕ полагайся на то, в какое поле файл "
    "прикреплён (сотрудники ошибаются) — смотри само содержимое. Извлеки то, что реально есть: "
    "contract_no/contract_date = № и дата договора (если договор есть); "
    "total = сумма договора ИЛИ счёта ИМЕННО по этому документу (НЕ накопительный итог по объекту); "
    "currency, nds (режим НДС); аванс (% и сумму); гарантийное удержание (% и сумму); бартер (есть/нет и сумму); "
    "vypolneno_sum = сумма ПРИНЯТЫХ работ по АВР/КС-2/КС-3, если акт есть (иначе 0); "
    "ochered = очередь/блок; object = ЖК (Атмосфера/Аура/Керуен/Аксай/…); account = № счёта/акта; "
    "bin = БИН/ИИН подрядчика/поставщика (ровно 12 цифр из реквизитов; НЕ ИИК/IBAN 'KZ…', НЕ БИК); "
    "article = ВИД РАБОТ/поставки короткой фразой как в смете: 'Монолитные работы','Фундамент','Земляные работы',"
    "'Кровля','Фасад','Внутренняя отделка','Окна и двери','Кладка стен','Инженерные сети','Лифтовое оборудование',"
    "'Поставка арматуры/материалов','Аренда техники','Проектные работы'. "
    "doc_kinds = какие типы документов реально увидел, через запятую (напр. 'АВР, КС-2, КС-3' или 'договор, счёт'). "
    "В notes — важные условия/оговорки одной строкой. Чего нет — ставь 0 или пустую строку.")
